    ...
    for grid_size in sorted(all_q_tables.keys()):
        ...
        for state, q_values in q_table.items():
            if state not in merged_q_table:
                merged_q_table[state] = (q_values.copy(), 1)
            else:
                sum_values, count = merged_q_table[state]
                merged_q_table[state] = (sum_values + q_values, count + 1)

    final_q_table = {state: q_sum / count for state, (q_sum, count) in merged_q_table.items()}
